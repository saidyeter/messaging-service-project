<script>
    ...
    // }
	import { connect } from '$lib/helper/socket-helper.js';
	import {  opponentList } from '$lib/helper/store';
	import { apiGetOpponents, apiGetUserInfo } from '$lib/helper/api-helper.js';
	import Chat from './chat.svelte';
	let chatbox;
	let opponenUserName = '';
	onMount(() => { 
		getOpponents();
		connect(onNewMessage, reConnect);
	});
	...
	async function getOpponents() { 
		const opponents = await apiGetOpponents(); 
		if (opponents) {
			$opponentList = [...opponents.sort((a, b) => (a.lastMessaged > b.lastMessaged ? -1 : 1))]; 
		}
...
console.log(opponenUserName);
	...
	async function handleNewMessage(event) {
		...
		try {
			const userdata = await apiGetUserInfo( opponent);
	...
	{#if shown}
		<div class="contact-list" trasition:fly>
			...
			<div class="new-message">
				<input type="text" bind:value={opponent} placeholder="username for new message" />
